...
//minified these arrays to save some space as they don't have much info to be explaned
...
function generator(adj, noun) {
  ...
  var randomAdjective = parseInt(random * adjectives.length);
  var randomNoun = parseInt(random * nouns.length);
...
function randomName() {
  var random=Math.random();
  var randomNumberAdj = parseInt(random * adjectives.length);
  var randomNumberNoun = parseInt(random * nouns.length);
...
var pizzass=document.getElementById("randomPizzas");
for (var i = 2; i < 100; i++) {
  var pizzasDiv = pizzass;
...
function updatePositions() {
  ...
  var hard_calculations=document.body.scrollTop / 1250;
  var items = document.getElementsByClassName('mover');
  for (var i = 0; i < items.length; i++) {
    var phase = Math.sin(hard_calculations + (i % 5));
...
document.addEventListener('DOMContentLoaded', function() {
  var cols = 8;
  var s = 256;
  var movingPizzas2=getElementById("movingPizzas1");
  ...
  for (var i = 0; i <pizza_per_page; i++) {
    ...
    elem.style.top = (cal * s) + 'px';
    document.movingPizzas2.appendChild(elem);
  }
  updatePositions();
});
